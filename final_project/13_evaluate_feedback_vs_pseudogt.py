...
LLM_PARAM_DIR = os.path.join(BASE_DIR, "outputs", "llm_predictions_search")
LLM_TRAJ_DIR = os.path.join(BASE_DIR, "outputs", "llm_trajectories_search")
LLM_OUTCOME_DIR = os.path.join(BASE_DIR, "outputs", "llm_outcomes_search")
...
def main():
    pred_files = [
        ...
        if f.endswith("_search.json")
    ]

    rows = []

    for fname in pred_files:
        case_id = fname.replace("_search.json", "")

        gt_param_path = os.path.join(PSEUDO_PARAM_DIR, f"{case_id}_carla.json")
        pred_param_path = os.path.join(LLM_PARAM_DIR, f"{case_id}_search.json")
        ...
        row = {
            "case_id": case_id,
            "method": "HF TinyLlama Few-shot + CARLA Feedback",
        }
    ...
    out_csv = os.path.join(OUT_DIR, "search_vs_pseudogt_eval.csv")
    ...
    if len(df) > 0:
        ...
        print("No feedback evaluation rows found.")
