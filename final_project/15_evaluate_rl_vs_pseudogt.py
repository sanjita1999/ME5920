...
LLM_PARAM_DIR = os.path.join(BASE_DIR, "outputs", "llm_predictions_rl")
LLM_TRAJ_DIR = os.path.join(BASE_DIR, "outputs", "llm_trajectories_rl")
LLM_OUTCOME_DIR = os.path.join(BASE_DIR, "outputs", "llm_outcomes_rl")
...
def main():
    pred_files = sorted([
        ...
        if f.endswith("_rl.json")
    ])

    rows = []

    for fname in pred_files:
        case_id = fname.replace("_rl.json", "")

        gt_param_path = os.path.join(PSEUDO_PARAM_DIR, f"{case_id}_carla.json")
        pred_param_path = os.path.join(LLM_PARAM_DIR, f"{case_id}_rl.json")
        ...
        row = {
            "case_id": case_id,
            "method": "HF TinyLlama Few-shot + CARLA RL Search",
        }
    ...
    out_csv = os.path.join(OUT_DIR, "rl_vs_pseudogt_eval.csv")
    ...
    if len(df) > 0:
        ...
        print("No RL evaluation rows found.")
